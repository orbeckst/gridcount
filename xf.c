...
bool xf_write_XFarbe (int ncolours) {
  ...
  fpxfi("y-annotation-incr",0);  /* described in */
...
FILE *xf_open (const char *fname,const char *header,int nx,int ny) {      
...
void xf_append_axes_annotation (FILE *fData, real x1, real x2,real y1, real y2) {
  fprintf(fData,"%f  %f\n",x1, x2);
  fprintf(fData,"%f  %f\n",y1, y2);
  return;
}
